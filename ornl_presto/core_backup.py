...
def get_noise_generators():
    ...
    return {
        ...
        "PercentilePrivacy": lambda d, epsilon, **kw: percentilePrivacy(
            d, percentile=kw.get("percentile", 50)
        ),
...
def dp_target(
    ...
    # apply DP to ML
    model, optimizer, criterion, train_dataloader, privacy_engine = dp_function(
        noise_multiplier, max_grad_norm, model_class, train_dataset, X_test
    )
    # compute accuracy
    predicted_classes, accuracy = dp_function_train_and_pred(
        model, optimizer, criterion, train_dataloader, X_test, y_test
    )
...
def dp_pareto_front(x1, x2, model_class, X_test, train_dataset, y_test):
    ...
    X_params = np.asarray([[100.0, 1.0], [10, 1.0], [1.1, 1.0], [0.1, 0.1]])
    Y_params = np.asarray([[-0.01, 0.51], [-0.09, 0.84], [0.62, 0.865], [650, 0.885]])
    ...
    for i in range(100):
        ...
        training_iter = 50
        mean1, Cov1 = gpr_gpytorch(
            X_params, Y_params[:, 0], Xg[unmeasured_points, :], training_iter
        )
        mean2, Cov2 = gpr_gpytorch(
            X_params, Y_params[:, 1], Xg[unmeasured_points, :], training_iter
        )

        Z1 = np.random.multivariate_normal(
            mean1.detach().numpy().flatten(), Cov1.detach().numpy(), 1
        ).T
        Z2 = np.random.multivariate_normal(
            mean2.detach().numpy().flatten(), Cov2.detach().numpy(), 1
        ).T
        ...
        accuracy, privacy_engine, predicted_classes = dp_target(
            Xg[pt, 0], Xg[pt, 1], model_class, X_test, train_dataset, y_test
        )
...
def dp_hyper(model_class, train_dataset, test_dataset, seed=42):
    ...
    optimizer = BayesianOptimization(
        f=bo_objective, pbounds=pbounds, random_state=seed, verbose=2
    )
